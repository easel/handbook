...
Once you are know what task you are taking on, you should create a feature branch. It is very important
to always start with a clean slate, so you should always run `get fetch upstream && git reset --hard upstream/master`
...
Once you have a clean place to start, create a new branch and switch to it using the `git checkout -b my_new_branch` 
command. The branch name, in this case `my_new_branch` should be descriptive of the work you intend to do.

TIP: Always start from the same SHA hash as the upstream master
